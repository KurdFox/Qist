  ...
  return (
    <div className="space-y-8">
      <header className="flex items-center gap-4">
        <button 
          onClick={() => navigate(-1)}
          className="p-3 bg-white rounded-2xl border border-gray-100 text-gray-400 hover:text-blue-600 hover:border-blue-100 transition-all shadow-sm"
        >
          <ArrowLeft size={20} />
        </button>
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">هەموو چالاکییەکان</h1>
          <p className="text-gray-500 font-medium">مێژووی هەموو گۆڕانکاری و چالاکییەکان</p>
        </div>
      </header>
        ...
        <div className="space-y-8">
          {activities.length === 0 ? (
            <div className="text-center py-20">
              ...
              <p className="text-gray-400 font-bold">هیچ چالاکییەک نییە بۆ پیشاندان</p>
            </div>
          ) : (
            activities.map((activity, i) => (
              <motion.div 
                ...
                key={activity.id} 
                className="flex items-center gap-6 p-6 rounded-3xl hover:bg-gray-50 transition-all border border-transparent hover:border-gray-100 group"
              >
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <p className="text-lg font-bold text-gray-900">{activity.title}</p>
                    <span className="text-xs font-bold text-gray-300">•</span>
                    <p className="text-sm font-bold text-gray-400">
                      {formatDistanceToNow(new Date(activity.timestamp), { addSuffix: true, locale: ckb })}
                    </p>
                  </div>
                  <p className="text-gray-500 font-medium">{activity.subtitle}</p>
                </div>
                {activity.amount && (
                  <div className="text-left">
                    <p className={`text-lg font-black ${activity.type === 'installment' ? 'text-green-600' : 'text-blue-600'}`}>
